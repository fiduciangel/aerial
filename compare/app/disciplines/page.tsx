  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 sm:py-10">
          ...
          >
            <div className="flex flex-col gap-5 p-5 sm:flex-row sm:p-6">
              ...
              <div className="sm:w-48 sm:shrink-0">
                ...
                <p className="mt-0.5 text-sm text-slate-400">{d.en}</p>

                {/* Intensity meter */}
                <div className="mt-3">
                  <div className="flex items-center justify-between text-xs text-slate-500">
                    <span>體能強度</span>
                    <span className="font-semibold text-slate-700">{d.intensityLabel}</span>
                  </div>
                    ...
                    {Array.from({ length: 5 }).map((_, i) => (
                      <span
                        ...
                        className={`h-1.5 flex-1 rounded-full ${
                          i < d.intensity ? d.accent.bar : "bg-slate-200"
                        }`}
                      />
                    ))}
                  </div>
              ...
              <div className="flex-1">
                <p className="font-medium text-slate-700">{d.oneLiner}</p>
                <p className="mt-2 leading-relaxed text-slate-600">{d.intro}</p>
                ...
                <p className="mt-3 text-sm text-slate-500">
                  <span className="font-semibold text-slate-600">適合誰:</span> {d.goodFor}
                </p>
              </div>
            </div>
          </article>
      ...
      <div className="mt-10 rounded-2xl border border-brand-200 bg-brand-50 p-6 text-center">
        <h2 className="text-lg font-bold text-slate-900">找到有興趣的項目了嗎?</h2>
